...
void PCU_SetDirection (PCU_Type * PCUx, uint32_t pin_no, PORT_Type dir_type)
{
	...
	//------------------------------------------------------------------------------
	reg_val = PCUx->CR;
	reg_val &= ~(PCU_CR_MASK << pin_offset);
	reg_val |= ((dir_type & PCU_CR_MASK) << pin_offset);
...
//************************************************************************************
//*/
//void PCU_ConfigurePullup (PCU_Type * PCUx, uint32_t pin_no, PUPD_Type pullup)
//{
...
void PCU_ConfigurePullupdown (PCU_Type * PCUx, uint32_t pin_no, PUPD_Type mode)
{
	...
	case PULLDOWN_ENABLE:
						reg_val |= (0x01UL << (pin_no + 16));
	case PULLUP_ENABLE:
						reg_val |= (0x01UL << pin_no);
						break;
	}
